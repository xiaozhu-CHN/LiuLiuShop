	...
	<body>
						...
						<div class="xm-edit-addr-box" id="J_editAddrBox">
							<div class="bd">
								<div class="item">
									...
									<input type="text" value="${(userAdressinfo.usadName)!}" name="userAddress[consignee]" id="Consignee" class="input" placeholder="收货人姓名" maxlength="15"
									 autocomplete='off'>
									<p class="tip-msg tipMsg"></p>
								</div>
								<div class="item">
									<label>联系电话<span>*</span></label>
									<input type="text" value="${(userAdressinfo.usadPhone)!}" name="userAddress[tel]" class="input" id="Telephone" placeholder="11位手机号" autocomplete='off'>
								...
								<div class="item">
									...
									<textarea name="userAddress[street]" class="input-area" id="Street" placeholder="路名或街道地址，门牌号">${(userAdressinfo.usadAdress)!}</textarea>
									<p class="tip-msg tipMsg"></p>
								</div>
								<div class="item">
									<label>邮政编码<span>*</span></label>
									<input type="text" value="${(userAdressinfo.usadEms)!}" name="userAddress[zipcode]" id="Zipcode" class="input" placeholder="邮政编码" autocomplete='off'>
									<p class="zipcode-tip" id="zipcodeTip"></p>
									<p class="tip-msg tipMsg"></p>
								</div>
							</div>
							<div class="ft clearfix">
								<button type="button" class="btn btn-lineDake btn-small" onclick="cancelupdate()">取消</button>
								<button type="button" class="btn btn-primary  btn-small" onclick="updateUserAdreeinfo(${userAdressinfo.usadId})">修改</button>
							</div>
						</div>
					</div>
				</div>
		...
		<script type="text/javascript">
		    function cancelupdate(){
		    	window.location.href="/user/userAdressinfoFindByUserId";
		    	
		    }
		    var usad_id;
		    function updateUserAdreeinfo(usadId){
		    	usad_id = usadId;
				...
				if(usad_name.length<=0)$("#msg").html("收货人不能为空!");
					...
					$.ajax({
			            type: "POST",
			            url: "/user/userAdressinfoUpdateByUsadId",
			            data: {
			            	usadId:usad_id,
			            	usadName:usad_name,
			            	usadPhone:usad_phone,
			            	areaId:area_name,
			            	usadAdress:usad_adress,
			            	usadEms:user_ems,			          
			            	success: function(){				            	
				            		alert("修改成功！");
				            		window.location.href="/user/toAddress"; 				            		
				            },
				            error:function() { $("#msg").html("修改错误");}
			            },
